...
public class CSV
{
	...
	public static final ContentType CSV_CONTENT_TYPE=ContentType.getInstance(ContentType.TEXT_PRIMARY_TYPE, CSV_SUBTYPE);
	...
	*/
	...
	/**Appends a record to the given file, first adding the provided headers
	(if any) if the file does not yet exist. The information is encoded using
	UTF-8.
	@param file The CSV file.
	@param headers The headers with which to initialize the file, or
		<code>null</code> if no headers are desired.
	...
	*/
	public static void appendRecord(final File file, final String[] headers, final Object[] objects) throws IOException
	{
		...
		final BufferedWriter bufferedWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file, true), CharacterEncoding.UTF_8));
		try
		{
